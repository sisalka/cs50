...
void tabulate(void)
{
    ...
    for (voter=0; voter < voter_count; voter++){
        ...
        while (candidates[c].eliminated == true && rank < candidate_count){
            rank ++;
...
bool print_winner(void)
{
    ...
    //if (voter_count %2 == 0){
    //    majority_count = (voter_count/2)+ 1;
    //}
    //else {
    //    majority_count = (voter_count)/2)+ 0.5;
    //}

    for (int c =0; c< candidate_count; c++){
        if (candidates[c].votes >= majority_count) {
            printf("The winner is %s, with the number of votes %d \n", candidates[c].name, candidates[c].votes);
            return true;
        }
    }
    return false;
...
bool is_tie(int min)
{
    ...
    int neelimin = 0;
    for (int c= 0; c< candidate_count; c++){
        if (min == candidates[c].votes && candidates[c].eliminated == false){
            n++;
        }
